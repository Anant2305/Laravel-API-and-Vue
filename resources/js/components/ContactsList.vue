    ...
    <div>
        ...
        <div v-else>
            ...
            <div v-for="contact in contacts">
                <router-link :to="'/contacts/' + contact.data.contact_id" class="flex items-center border border-grey-400 p-4 hover:bg-grey-100">
                    <UserCircle :name="contact.data.name" />

                    <div class="pl-4">
                        <p class="text-blue-400 font-bold">{{contact.data.name}}</p>
                        <p class="text-grey-600">{{contact.data.company}}</p>
                    </div>
                </router-link>
            </div>
